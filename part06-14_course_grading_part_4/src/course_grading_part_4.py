...
with open('results.txt', 'w') as res_file, open(course_info) as course_info, open('results.csv', 'w') as res_csv_file:
    course_credits = [line.split(':')[1].strip() for line in course_info.readlines()]
    header = f'{course_credits[0]}, {course_credits[1]} credits\n'
    res_file.write(header)
    res_file.write((len(header)-1)*'='+'\n')
    res_file.write(f'{'name':30}{'exec_nbr':10}{'exec_pts.':10}{'exm_pts.':10}{'tot_pts.':10}{'grade':10}\n')
    for person in db.keys():
        res_file.write(f'{db[person]['full name'].split('\n')[0]:30}{db[person]['ex']:<10}{db[person]['ex_p']:<10}{db[person]['exam_p']:<10}{db[person]['total']:<10}{db[person]['grade']:<10}\n')
        res_csv_file.write(f'{person};{db[person]['full name'].split('\n')[0]};{db[person]['grade']}\n')
